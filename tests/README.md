...
This will download the datasets to `tests/data/external/` and prepare them using an adaptive duration strategy:
- **Short samples (< 5s)**: Looped to reach exactly 5 seconds.
- **Medium samples (5-10s)**: Preserved in full.
- **Long samples (> 10s)**: Trimmed to a 10-second center segment.

Speech samples (TCD-VOIP) are resampled to 16kHz mono, while audio samples (PMLT2014, SoundExpert) use 48kHz stereo.
