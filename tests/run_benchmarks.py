...
def run_visqol(ref_wav, deg_wav, mode):
    ...
    try:
        subcmd = "wideband" if mode == "speech" else "fullband"
        cmd = ["visqol", "--reference_file", ref_wav, "--degraded_file", deg_wav, subcmd]
        if subcmd == "fullband":
            cmd.extend(["--similarity_to_quality_model", "model/libsvm_nu_svr_model.txt"])
                ...
                try:
                    ...
                    try:
                        ...
                        if os.path.exists(deg_wav):
                            ...
                            v_ref = input_path + f".{precision}.vref.wav"
                            v_deg = deg_wav + f".{precision}.vdeg.wav"
                            ...
                            if os.path.exists(v_ref): os.remove(v_ref)
                            if os.path.exists(v_deg): os.remove(v_deg)
                    except:
                        pass
                    finally:
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python3 tests/run_benchmarks.py <faac_bin_path> <lib_path> <precision_name> <output_json> [--perceptual] [--coverage 100]")
        sys.exit(1)
