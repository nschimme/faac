...
/*
 * Modern libfaac encoder API.
 *
 * This is the recommended surface for new code. It is a self-contained,
 * additive layer over the encoder; the classic faacEnc* API in <faac.h>
 * remains supported unchanged for existing callers. New code should prefer
 * the faac_* API here.
 *
...
/* Opaque encoder handle, distinct from the legacy void* faacEncHandle so the
 * two APIs cannot be mixed by accident. */
...
typedef enum faac_status {
    ...
    FAAC_ERR_BAD_STATE        = -6,  /* call made in a state that does not permit it */
...
typedef struct faac_params {
    ...
    uint32_t                num_channels;  /* channel count, 1..faac_max_channels() (required) */
...
/* Highest num_channels this build accepts (governed by the build-time
 * max-channels option). */
FAACAPI uint32_t faac_max_channels(void);

/*
 * Library version / copyright strings. Both out-params are optional (pass NULL
 * to skip). The returned strings are static and library-owned; do not free.
 */
