    ...
    for (int sfb = sfmin; sfb < coder->sfbn; sfb++) {
        ...
        if ((min(enrgl, enrgr) * thrmid) >= max(enrgs, enrgd)) {
            ...
            if (ms) {
                for (int win = wstart; win < wend; win++) {
                    faac_real * __restrict sl = sl0 + win * 128 + start;
                    faac_real * __restrict sr = sr0 + win * 128 + start;
                    for (int l = 0; l < len; l++) {
                        if (phase == PH_IN) { faac_real s = sl[l] + sr[l]; sl[l] = 0.5 * s; sr[l] = 0; }
        ...
        if (min(enrgl, enrgr) <= (thrside * max(enrgl, enrgr))) {
            if (enrgl < enrgr) {
                for (int win = wstart; win < wend; win++) {
                    faac_real * __restrict sl = sl0 + win * 128 + start;
                    for (int l = 0; l < len; l++) sl[l] = 0.0;
                }
            } else {
                for (int win = wstart; win < wend; win++) {
                    faac_real * __restrict sr = sr0 + win * 128 + start;
                    for (int l = 0; l < len; l++) sr[l] = 0.0;
                }
            }
        }
    ...
    for (int sfb = sfmin; sfb < cl->sfbn; sfb++) {
        ...
        if ((min(enrgl, enrgr) * thrmid) >= max(enrgs * 0.25, enrgd * 0.25)) {
            ...
            if (ms) {
                ...
                for (int win = wstart; win < wend; win++) {
                    faac_real * __restrict sl = sl0 + win * 128 + start, * __restrict sr = sr0 + win * 128 + start;
                    if (phase == PH_IN) for (int l = 0; l < len; l++) { faac_real sum = sl[l] + sr[l]; sl[l] = 0.5 * sum; sr[l] = 0.0; }
                    else for (int l = 0; l < len; l++) { faac_real diff = sl[l] - sr[l]; sr[l] = 0.5 * diff; sl[l] = 0.0; }
                }
            }
        }
