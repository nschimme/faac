...
void TnsInit(faacEncStruct* hEncoder)
{
    ...
    for (ch = 0; ch < hEncoder->numChannels; ch++) {
        ...
        info->tnsMaxBandsLong = (uint8_t)tns_sfb_range[fs].max;
        info->tnsNumSwbLong = (uint8_t)hEncoder->srInfo->num_cb_long;
        info->tnsMinBandNumberLong = (uint8_t)tns_sfb_range[fs].min;
    }
}
