...
#define TNS_CALIBRATION     0.90    /* Calibration factor against corpus anchor */
...
void TnsInit(faacEncStruct* hEncoder)
{
    ...
    for (channel = 0; channel < hEncoder->numChannels; channel++) {
        ...
        /* Order 12 captures the dominant spectral resonances. */
        ...
        /* Short-window gain threshold. Formula accounts for 8 windows. */
        {
            int frame_bits    = (int)(effectiveBitratePerCh * FRAME_LEN
                                      / hEncoder->sampleRate);
            int spectral_bits = (int)(frame_bits * TNS_SPECTRAL_FRAC);
            int tns_overhead  = (tnsInfo->tnsMaxOrderShort * DEF_TNS_COEFF_RES
                                + TNS_FIXED_OVERHEAD) * MAX_SHORT_WINDOWS;
            int denom = spectral_bits - tns_overhead;
            faac_real thresh;
            if (denom <= 0) {
                thresh = (faac_real)TNS_THRESH_CAP;
            ...
            tnsInfo->gainThreshShort = thresh;
        }
    }
}
               ...
               faac_real* temp)
{
    ...
    case ONLY_SHORT_WINDOW :
        ...
        stopBand = maxSfb; /* Sync with decoder */
