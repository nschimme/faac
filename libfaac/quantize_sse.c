 ...
 * SSE2-optimized quantization routines
...
/* Branchless SIMD max for signed 32-bit integers in SSE2 */
static inline __m128i mm_max_epi32_sse2(__m128i a, __m128i b)
{
    __m128i mask = _mm_cmpgt_epi32(a, b);
    return _mm_or_si128(_mm_and_si128(mask, a), _mm_andnot_si128(mask, b));
}

/* Vectorized x^(3/4) quantization: also tracks 'maxq' to avoid an O(N)
 * spectral rescan just to pick a Huffman codebook. */
int quantize_sse2(const float * __restrict xr, int * __restrict xi, int n, float sfacfix)
{
    ...
    __m128i maxq_vec = _mm_setzero_si128();
    int cnt = 0;
    ...
    for (; cnt <= n - 4; cnt += 4)
    {
        ...
        maxq_vec = mm_max_epi32_sse2(maxq_vec, xi_vec);
    ...
    int mq_vals[4];
    _mm_storeu_si128((__m128i*)mq_vals, maxq_vec);
    int maxq = mq_vals[0];
    if (mq_vals[1] > maxq) maxq = mq_vals[1];
    if (mq_vals[2] > maxq) maxq = mq_vals[2];
    if (mq_vals[3] > maxq) maxq = mq_vals[3];
