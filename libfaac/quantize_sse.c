...
{
    int win, cnt;
    const __m128 vzero32 = _mm_setzero_ps();
    ...
    for (win = 0; win < gsize; win++)
    {
        ...
        for (cnt = 0; cnt <= end - 4; cnt += 4)
        {
            ...
            __m128 t = _mm_add_ps(_mm_sqrt_ps(_mm_mul_ps(_mm_mul_ps(_mm_and_ps(x, abs_mask), vsfac), _mm_sqrt_ps(_mm_mul_ps(_mm_and_ps(x, abs_mask), vsfac)))), vmagic);
            __m128i s = _mm_srai_epi32(_mm_castps_si128(x), 31);
            _mm_storeu_si128((__m128i*)(xi + cnt), _mm_sub_epi32(_mm_xor_si128(_mm_cvttps_epi32(t), s), s));
        }
#else
        ...
        for (cnt = 0; cnt <= end - 4; cnt += 4)
        {
            ...
            __m128d t0 = _mm_add_pd(_mm_sqrt_pd(_mm_mul_pd(_mm_mul_pd(_mm_and_pd(x0, dabs_mask), vdsfac), _mm_sqrt_pd(_mm_mul_pd(_mm_and_pd(x0, dabs_mask), vdsfac)))), vdmagic);
            __m128d t1 = _mm_add_pd(_mm_sqrt_pd(_mm_mul_pd(_mm_mul_pd(_mm_and_pd(x1, dabs_mask), vdsfac), _mm_sqrt_pd(_mm_mul_pd(_mm_and_pd(x1, dabs_mask), vdsfac)))), vdmagic);
