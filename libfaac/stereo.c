                  ...
                  )
{
    ...
    if (!phthr) return;
    phthr = 1.0 / phthr;
    if (cl->block_type == ONLY_SHORT_WINDOW) sfmin = 1; else sfmin = 8;
    for (sfb = 0; sfb < sfmin; sfb++) (*sfcnt)++;
    for (sfb = sfmin; sfb < cl->sfbn; sfb++)
    {
        ...
        for (win = wstart; win < wend; win++) {
            for (int l = 0; l < len; l++) {
                enrgl += sl_ptr[l] * sl_ptr[l];
                enrgr += sr_ptr[l] * sr_ptr[l];
                enrglr += sl_ptr[l] * sr_ptr[l];
            }
            sl_ptr += BLOCK_LEN_SHORT; sr_ptr += BLOCK_LEN_SHORT;
        }
        enrgs = enrgl + enrgr + 2.0 * enrglr;
        enrgd = enrgl + enrgr - 2.0 * enrglr;
        if (enrgs < 0.0) enrgs = 0.0; if (enrgd < 0.0) enrgd = 0.0;
        ...
        if (hcb != HCB_NONE) {
            ...
            if (pan > 30) { cl->book[*sfcnt] = HCB_ZERO; (*sfcnt)++; continue; }
            if (pan < -30) { cr->book[*sfcnt] = HCB_ZERO; (*sfcnt)++; continue; }
            cl->sf[*sfcnt] = sf;
            cr->sf[*sfcnt] = -pan;
            cr->book[*sfcnt] = hcb;
                   ...
                   )
{
    ...
    if (coder->block_type == ONLY_SHORT_WINDOW) sfmin = 1; else sfmin = 8;
    ...
    for (sfb = sfmin; sfb < coder->sfbn; sfb++) {
        ...
        for (win = wstart; win < wend; win++) {
            for (int l = 0; l < len; l++) {
                enrgl += sl_ptr[l] * sl_ptr[l]; enrgr += sr_ptr[l] * sr_ptr[l]; enrglr += sl_ptr[l] * sr_ptr[l];
            }
            sl_ptr += BLOCK_LEN_SHORT; sr_ptr += BLOCK_LEN_SHORT;
        }
        ...
        if ((min(enrgl, enrgr) * thrmid) >= max(enrgs, enrgd)) {
            ms = 1;
            if (enrgs > (enrgd * coll_thr)) apply_ms_mono(sl0, sr0, start, len, wstart, wend, 1);
            else if (enrgd > (enrgs * coll_thr)) apply_ms_mono(sl0, sr0, start, len, wstart, wend, 0);
            else apply_ms_true(sl0, sr0, start, len, wstart, wend);
        }
                ...
                )
{
    ...
    for (sfb = sfmin; sfb < cl->sfbn; sfb++) {
        ...
        if ((min(enrgl, enrgr) * thrmid) >= max(enrgs * 0.25, enrgd * 0.25)) {
            ms = 1; msused = 1;
            if (enrgs * 0.25 > (enrgd * 0.25 * coll_thr)) apply_ms_mono(sl0, sr0, start, len, wstart, wend, 1);
            else if (enrgd * 0.25 > (enrgs * 0.25 * coll_thr)) apply_ms_mono(sl0, sr0, start, len, wstart, wend, 0);
            else apply_ms_true(sl0, sr0, start, len, wstart, wend);
        }
        if (!ms && (min(enrgl, enrgr) <= (thrside * max(enrgl, enrgr)))) {
            if (enrgl < enrgr) zero_channel(sl0, start, len, wstart, wend);
...
{
    int chn;
    static const faac_real thr075 = 1.09 - 1.0, thrmax = 1.25 - 1.0, isthrmax = M_SQRT2 - 1.0;
    faac_real sidemin = 0.05, sidemax = 0.2, thrmid, thrside, isthr, coll_thr = 1.0;
    int is_freq = 5500;

    const char *env_coll = getenv("FAAC_STEREO_MS_COLL_THR");
    if (env_coll) coll_thr = atof(env_coll);

    const char *env_f = getenv("FAAC_STEREO_IS_FLOOR");
    if (env_f) is_freq = atoi(env_f);

    thrmid = 1.0; thrside = 0.0; isthr = 1.0;
    switch (mode) {
    case JOINT_MIXED:
        thrmid = (thr075 * 0.85) / quality; if (thrmid > thrmax) thrmid = thrmax;
        thrside = sidemin / quality; if (thrside > sidemax) thrside = sidemax;
        thrmid += 1.0; isthr = 0.18 / quality + 1.0; if (isthr > isthrmax + 1.0) isthr = isthrmax + 1.0;
        break;
    case JOINT_MS:
        ...
        thrmid += 1.0; break;
    case JOINT_IS:
        isthr = 0.18 / (quality * quality) + 1.0; if (isthr > isthrmax + 1.0) isthr = isthrmax + 1.0;
        break;
    }
    thrmid *= thrmid; thrside *= thrside; isthr *= isthr;
    for (chn = 0; chn < maxchan; chn++) {
        CoderInfo *cp = coder + chn; if (!channel[chn].present) continue;
        for (int i = 0; i < cp->groups.n * cp->sfbn; i++) { cp->book[i] = HCB_NONE; cp->sf[i] = 0; }
    }
    for (chn = 0; chn < maxchan; chn++) {
        ...
        for (int i = 0; i < coder[chn].groups.n; i++) if (coder[chn].groups.len[i] != coder[rch].groups.len[i]) { channel[chn].common_window = 0; goto skip; }
        ...
        if (mode == JOINT_MIXED) {
            ...
            int cap = (sampleRate * 7) / 20; if (is_freq > cap) is_freq = cap;
            is_start_sfb = coder[chn].sfbn;
            for (int sfb = 0; sfb < coder[chn].sfbn; sfb++) {
                if ((coder[chn].sfb_offset[sfb] * sampleRate) / mdctlen >= is_freq) { is_start_sfb = sfb; break; }
            }
        }
        for (int group = 0; group < coder[chn].groups.n; group++) {
            int end = start + coder[chn].groups.len[group];
            if (mode == JOINT_MS) midside(coder + chn, channel + chn, s[chn], s[rch], &sfcnt, start, end, thrmid, thrside, coll_thr);
            else if (mode == JOINT_IS) stereo(coder + chn, coder + rch, s[chn], s[rch], &sfcnt, start, end, isthr);
            else if (mode == JOINT_MIXED) msused |= mixed(coder + chn, coder + rch, channel + chn, s[chn], s[rch], &sfcnt, start, end, thrmid, thrside, isthr, is_start_sfb, coll_thr);
            else sfcnt += coder[chn].sfbn;
            start = end;
        }
