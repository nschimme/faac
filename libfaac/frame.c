                          ...
                          )
{
    ...
    /* Force TNS off at very low per-channel bitrates. The TNS data
       ...
       of the spectral-coding budget at <=16 kbps/ch, and content at
       ...
       operating point instead. Threshold is set just above 16 kbps
       so voip 16 kbps mono is caught while any scenario at 17 kbps
       or above keeps the TNS wins (b5e97ce at 20000 lost 211 wins
       and dropped avg MOS by 0.026; 17000 narrows the gate to the
       single scenario known to regress). */
    if (hEncoder->config.bitRate && hEncoder->config.bitRate < 17000)
        useTns = 0;
