...
/*
 * Modern faac_* encoder API (see include/faac_encoder.h).
 *
 * This is a thin, well-defined facade over the classic faacEnc* core in
 * frame.c: the legacy encode path is reused verbatim (so its output is
 * unchanged) while this layer supplies the modern conveniences -- a single
 * params-at-open entry point, uniform faac_status error reporting, resolved
 * property queries, and library-owned ASC. The opaque faac_encoder* is the
 * same object as the legacy faacEncHandle (a faacEncStruct*), so the two APIs
 * share one implementation but expose distinct, non-interchangeable handles.
 */
...
/* The modern faac_input_format enumerators share their spelling with the legacy
 * FAAC_INPUT_* macros in <faaccfg.h> (pulled in by frame.h below). Snapshot the
 * enum values here, while the tokens still resolve to the enum, so the parity
 * _Static_assert further down actually compares enum against macro rather than
 * the macro against itself. */
enum {
    MODERN_INPUT_NULL  = FAAC_INPUT_NULL,  MODERN_INPUT_16BIT = FAAC_INPUT_16BIT,
    MODERN_INPUT_24BIT = FAAC_INPUT_24BIT, MODERN_INPUT_32BIT = FAAC_INPUT_32BIT,
    MODERN_INPUT_FLOAT = FAAC_INPUT_FLOAT
};
#include "frame.h"
...
_Static_assert(MODERN_INPUT_NULL  == FAAC_INPUT_NULL  && MODERN_INPUT_16BIT == FAAC_INPUT_16BIT
            && MODERN_INPUT_24BIT == FAAC_INPUT_24BIT && MODERN_INPUT_32BIT == FAAC_INPUT_32BIT
            && MODERN_INPUT_FLOAT == FAAC_INPUT_FLOAT, "input format drift");
...
FAACAPI uint32_t faac_max_channels(void)
{
    return (uint32_t)MAX_CHANNELS;
}

FAACAPI faac_status faac_version(const char **id, const char **copyright)
{
    char *vid = NULL, *vcopy = NULL;
    faacEncGetVersion(&vid, &vcopy);
    if (id)        *id        = vid;
    if (copyright) *copyright = vcopy;
    return FAAC_OK;
}
...
FAACAPI const char *faac_strerror(faac_status status)
{
    switch (status) {
        ...
        case FAAC_ERR_BAD_STATE:        return "operation not valid in this state";
