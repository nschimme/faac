                      ...
                      unsigned long bitRate, unsigned long sampleRate, int cbr)
{
    ...
    /* The reservoir is the decoder buffer (AAC_MAX_BITS_PER_CH per channel)
       refilled at the mean rate and drained a frame at a time. ABR/VBR leave
...
int RateControlFrameCap(RateControl *rc)
{
    ...
    /* A frame may spend mean + fill. Floor: a frame that would overflow the
       buffer is stuffed up to it, or content that cannot spend (pauses) leaves
       the stream far under its rate. Kept two bytes under the cap: fill is
       byte-granular, and at the ceiling bitrate (zero capacity) floor and cap
       meet. */
