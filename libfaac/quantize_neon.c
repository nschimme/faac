...
void quantize_sfb_neon(int end, int gsize, faac_real sfacfix, const faac_real *xr, int *xi)
{
    ...
    for (win = 0; win < gsize; win++)
    {
        ...
        for (cnt = 0; cnt <= end - 4; cnt += 4)
        {
            ...
            float32x4_t t = vaddq_f32(vsqrtq_f32(vmulq_f32(vmulq_f32(vabsq_f32(x), vsfac), vsqrtq_f32(vmulq_f32(vabsq_f32(x), vsfac)))), vmagic);
        ...
        for (cnt = 0; cnt <= end - 2; cnt += 2)
        {
            ...
            float64x2_t t = vaddq_f64(vsqrtq_f64(vmulq_f64(vmulq_f64(vabsq_f64(x), vdsfac), vsqrtq_f64(vmulq_f64(vabsq_f64(x), vdsfac)))), vdmagic);
            int32x2_t q = vqmovn_s64(vcvtq_s64_f64(t));
            int32x2_t g = vqmovn_s64(vreinterpretq_s64_u64(vcltq_f64(x, vzero64)));
