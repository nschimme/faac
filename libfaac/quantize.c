...
static void q_scalar(const faac_real *xr, int *xi, int n, faac_real f) {
    for (int i = 0; i < n; i++) {
        faac_real v = xr[i], t = FAAC_FABS(v) * f;
        t = FAAC_POW(t, 0.75);
        int q = (int)(t + 0.4054);
        if (q > 8191) q = 8191;
        xi[i] = (v < 0) ? -q : q;
...
static void count_bits_loop(CoderInfo *ci, int sfb, int trial_sfac, float *noise, int *bits) {
    ...
    faac_real f = (faac_real)pow(2.0, 0.25 * trial_sfac);
    int xi[BLOCK_LEN_LONG];
    if (n > BLOCK_LEN_LONG) n = BLOCK_LEN_LONG;

#if defined(HAVE_SSE2)
    if (get_cpu_caps() & CPU_CAP_SSE2) quantize_sse2(ci->xabs + start, xi, n, f);
    else q_scalar(ci->xabs + start, xi, n, f);
#else
...
void twoloop_exec(CoderInfo *ci, float *thr, int target) {
    int sfac_offsets[MAX_SCFAC_BANDS];
    int bits, total, best_mid;
    float noise;
...
#if defined(HAVE_SSE2)
        if (get_cpu_caps() & CPU_CAP_SSE2) quantize_sse2(ci->xabs + st, ci->xitab + st, n, f);
        else q_scalar(ci->xabs + st, ci->xitab + st, n, f);
#else
...
int BlocQuant(CoderInfo *ci, faac_real *xr, AACQuantCfg *cfg) {
    ...
    if (ci->sfbn > MAX_SCFAC_BANDS) ci->sfbn = MAX_SCFAC_BANDS;
    for (int i = 0; i < ci->sfbn; i++) {
        int st = ci->sfb_offset[i], en = ci->sfb_offset[i+1];
        float e = 0;
