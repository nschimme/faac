...
/**
 * Joint window grouping for CPE (Channel Pair Elements) across Left and Right channel spectral data.
 *
 * In ISO/IEC 14496-3 §4.5.2.3.5 and §4.5.2.3.6, M/S stereo and intensity stereo can only be
 * applied to scale factor bands of channel pairs when they share a common window sequence
 * (common_window == 1). Independent grouping causes grouping boundaries to drift,
 * dropping common_window to 0 and forcing individual channel coding at severe bitstream cost.
 *
 * This function enforces identical grouping on both channels.
 * Option 0 (Logical OR) monitors independent running minima and maxima for both channels.
 * It detects transients on either channel with full sensitivity, avoiding energy dilution that
 * would occur if loud steady-state signal in one channel masked a transient on the other.
 * Option 1 (Sum of Energies) acts on the combined spectral energy.
 */
void BlocGroupStereo(float * __restrict xrL, float * __restrict xrR,
                      CoderInfo * __restrict cl, CoderInfo * __restrict cr,
                      AACQuantCfg * __restrict cfg)
{
    ...
    float band_e_L[NSFB_SHORT], run_min_L[NSFB_SHORT], run_max_L[NSFB_SHORT];
    float band_e_R[NSFB_SHORT], run_min_R[NSFB_SHORT], run_max_R[NSFB_SHORT];
    int group_start = 0;
    ...
    for (int win = 0; win < MAX_SHORT_WINDOWS; win++)
    {
        ...
        /* Zero out high-frequency coefficients beyond cutoff in both channels */
        for (int k = cutoff; k < cl->sfb_offset[maxsfb]; k++)
        {
            wL[k] = 0.0f;
            wR[k] = 0.0f;
        }

        window_band_energy(cl, wL, GROUP_MIN_SFB, maxsfb, band_e_L);
        window_band_energy(cr, wR, GROUP_MIN_SFB, maxsfb, band_e_R);

        if (win == group_start)
        {
            for (int sfb = GROUP_MIN_SFB; sfb < maxsfb; sfb++)
            {
                if (JOINT_GROUP_MODE == 0)
                {
                    run_min_L[sfb] = run_max_L[sfb] = band_e_L[sfb];
                    run_min_R[sfb] = run_max_R[sfb] = band_e_R[sfb];
                }
                else
                {
                    run_min_L[sfb] = run_max_L[sfb] = band_e_L[sfb] + band_e_R[sfb];
                }
            }
            continue;
        }
        ...
        if (JOINT_GROUP_MODE == 0)
        {
            if (onset_votes_L > onset_quorum || onset_votes_R > onset_quorum) trigger = 1;
        }
        else
        {
            if (onset_votes_sum > onset_quorum) trigger = 1;
        }

        if (trigger)
        {
            int glen = win - group_start;
            cl->groups.len[cl->groups.n++] = glen;
            cr->groups.len[cr->groups.n++] = glen;
            group_start = win;
            for (int sfb = GROUP_MIN_SFB; sfb < maxsfb; sfb++)
            {
                if (JOINT_GROUP_MODE == 0)
    ...
    int final_len = MAX_SHORT_WINDOWS - group_start;
    cl->groups.len[cl->groups.n++] = final_len;
    cr->groups.len[cr->groups.n++] = final_len;
}
