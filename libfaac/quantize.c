                  ...
                  faac_real * __restrict bandenrg, faac_real * __restrict bandmaxe, int gnum, faac_real quality)
{
    ...
    for (sfb = 0; sfb < coderInfo->sfbn; sfb++) {
        ...
        faac_real inv_ref_avge = 1.0 / ref_avge;
        faac_real target = NOISETONE * FAAC_POW(avge * inv_ref_avge, 0.4);
        target += (1.0 - NOISETONE) * TONEMASK * FAAC_POW(maxe * inv_ref_avge, 0.4);
        if (coderInfo->block_type == ONLY_SHORT_WINDOW) target *= SHORT_PENALTY;
        target *= 10.0 / (1.0 + ((faac_real)(start + end) * inv_block_len));
