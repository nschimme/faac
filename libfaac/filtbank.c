...
void FilterBankInit(faacEncStruct* hEncoder)
{
    ...
    for( i=0; i<BLOCK_LEN_LONG; i++ )
        hEncoder->sin_window_long[i] = sinf((M_PI/(2*BLOCK_LEN_LONG)) * (i + 0.5));
    for( i=0; i<BLOCK_LEN_SHORT; i++ )
        hEncoder->sin_window_short[i] = sinf((M_PI/(2*BLOCK_LEN_SHORT)) * (i + 0.5));
...
static void CalculateKBDWindow(float* win, float alpha, int length)
{
    ...
    for(i=0; i<(length>>1); i++) {
        ...
        win[i] = Izero(alpha*sqrtf(1.0-tmp*tmp))*IBeta;
        sum += win[i];
    }
