...
void FilterBankInit(faacEncStruct* hEncoder)
{
    ...
    for (channel = 0; channel < hEncoder->numChannels; channel++) {
        ...
        hEncoder->backupFreqBuff[channel] = (faac_real*)AllocMemory(2*FRAME_LEN*sizeof(faac_real));
...
void FilterBankEnd(faacEncStruct* hEncoder)
{
    ...
    for (channel = 0; channel < hEncoder->numChannels; channel++) {
        ...
        if (hEncoder->backupFreqBuff[channel]) FreeMemory(hEncoder->backupFreqBuff[channel]);
