...
static void		MDCT_Internal				( faacEncStruct *hEncoder, faac_real *data, int N );
static void		IMDCT_Internal				( faacEncStruct *hEncoder, faac_real *data, int N );
...
void FilterBankInit(faacEncStruct* hEncoder)
{
    ...
    for( i=0; i<BLOCK_LEN_LONG; i++ )
        hEncoder->sin_window_long[i] = FAAC_SIN((M_PI/(2.0*BLOCK_LEN_LONG)) * (i + 0.5));
    for( i=0; i<BLOCK_LEN_SHORT; i++ )
        hEncoder->sin_window_short[i] = FAAC_SIN((M_PI/(2.0*BLOCK_LEN_SHORT)) * (i + 0.5));
                ...
                int overlap_select)
{
    ...
    case ONLY_LONG_WINDOW :
        ...
        MDCT_Internal( hEncoder, p_out_mdct, 2*BLOCK_LEN_LONG );
        break;
    ...
    case ONLY_SHORT_WINDOW :
        ...
        for ( k=0; k < MAX_SHORT_WINDOWS; k++ ) {
            ...
            MDCT_Internal( hEncoder, p_out_mdct, 2*BLOCK_LEN_SHORT );
...
void IFilterBank(CoderInfo *coderInfo,
                 ...
                 int overlap_select)
{
    /* sur: IFilterBank is unused in libfaac. Original logic kept. */
    ...
    case ONLY_LONG_WINDOW :
        ...
        /* MDCT/IMDCT signatures are kept internal or private to maintain API.
           Since IFilterBank is unused, we just leave it or use a default FFT_Tables if needed.
           But libfaac doesn't even use this. */
        break;
    }
...
static void MDCT_Internal( faacEncStruct* hEncoder, faac_real *data, int N )
{
    ...
    if (N == 2*BLOCK_LEN_LONG)
        twiddles = hEncoder->mdct_twiddles_long;
    else
...
static void IMDCT_Internal( faacEncStruct* hEncoder, faac_real *data, int N)
{
    ...
    switch (N) {
    case BLOCK_LEN_SHORT * 2:
        ffti( &hEncoder->fft_tables, xr, xi, 6);
        break;
    case BLOCK_LEN_LONG * 2:
        ffti( &hEncoder->fft_tables, xr, xi, 9);
    }
