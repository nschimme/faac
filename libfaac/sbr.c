...
/* Master frequency band table generation (ISO 14496-3:2009 §4.6.18.3.2).
 * start_min/stop_min (Table 4.84) and max_span (Table 4.85) are normative constants. */
...
static int compute_k2(int sampleRate, int kx, int bs_stop_freq)
{
    ...
    int prev = stop_min;
    /* Logarithmic distribution of remaining QMF bands to approximate a Bark/ERB scale. */
...
/* ISO 14496-3:2009 master_frequency_table_fs(): uniform dk-spaced edges kx..k2. */
static int build_freq_table(SBRInfo *sbr)
{
    ...
    int f_master[SBR_MAX_BANDS + 1], k2diff = (k2 - kx) - n_master * dk;
    ...
    {
        if (b > 0) f_master[b] += f_master[b - 1];
...
SBRInfo *SBRInit(int channels, int sampleRate, unsigned long bitRate, FFT_Tables *fft_tables)
{
    ...
    /* QMF pre-rotation and real-DFT twiddles (see qmf_analysis_64_slot_energy_fft). */
 ...
 * matches it to machine precision. Energy (magnitude squared) is sufficient:
 * the bitstream carries per-band magnitudes; the decoder reconstructs phase. */
static void qmf_analysis_64_slot_energy_fft(SBRInfo *sbr, const faac_real * __restrict ovl_pos, faac_real * __restrict energy, int kx, int k2)
{
    faac_real xr[64], xi[64];
    const faac_real * __restrict proto = qmf_c;
    for (int m = 0; m < 64; m++)
    {
        int n = 2 * m;
        faac_real a = proto[n]       * ovl_pos[639 - n]     + proto[n + 128] * ovl_pos[511 - n]
                    + proto[n + 256] * ovl_pos[383 - n]     + proto[n + 384] * ovl_pos[255 - n]
                    + proto[n + 512] * ovl_pos[127 - n];
        n++;
        faac_real b = proto[n]       * ovl_pos[639 - n]     + proto[n + 128] * ovl_pos[511 - n]
                    + proto[n + 256] * ovl_pos[383 - n]     + proto[n + 384] * ovl_pos[255 - n]
                    + proto[n + 512] * ovl_pos[127 - n];
    ...
    {
        int kr = 63 - k;
        faac_real Ar = 0.5f * (xr[k] + xr[kr]), Ai = 0.5f * (xi[kr] - xi[k]);
        faac_real Br = -0.5f * (xi[k] + xi[kr]), Bi = 0.5f * (xr[kr] - xr[k]);
...
{
    int slots = numSamples / SBR_QMF_BANDS_64, kx = sbr->kx, k2 = sbr->k2, nch = clamp_int(numChannels, 1, 2);
    int hslots = slots / 2 > 0 ? slots / 2 : 1, sampled = (slots - 1) / FAAC_SBR_DECIMATION + 1;
    faac_real bandHalfE[2][2][SBR_QMF_BANDS_64], slotEnergy[SBR_QMF_BANDS_64], tratio = 0.0f;
    faac_real workspace[SBR_QMF_OVL_LEN_64 + 2 * FRAME_LEN];

    memset(bandHalfE, 0, sizeof(bandHalfE));
    for (int ch = 0; ch < nch; ch++)
    {
        faac_real smax = 0.0f, ssum = 0.0f;
        ...
        {
            const faac_real * __restrict p_in = timeDomain[ch] + s * SBR_QMF_BANDS_64;
            faac_real stot = 0.0f;
            for (int n = 0; n < SBR_QMF_BANDS_64; n++) { faac_real v = p_in[n]; stot += v * v; }
            if (stot > smax) smax = stot;
            ssum += stot;

            if (s % FAAC_SBR_DECIMATION == 0)
            {
                qmf_analysis_64_slot_energy_fft(sbr, workspace + s * SBR_QMF_BANDS_64, slotEnergy, kx, k2);
                int h = clamp_int(s / hslots, 0, 1);
    ...
    /* Peak-to-mean slot power ratio: >transientThresh (~12 dB) triggers 2-envelope framing
     * to prevent temporal aliasing (pre-echo) in the SBR reconstruction. */
    sbr->numEnvelopes = (tratio > sbr->transientThresh) ? 2 : 1;
    sbr->eff_amp_res = (sbr->numEnvelopes == 1) ? 0 : sbr->bs_amp_res;
    int n_env = sbr->numEnvelopes, dlav = sbr->eff_amp_res ? 31 : 60;

    for (int ch = 0; ch < nch; ch++)
    {
        /* HIVAR (strongest) inverse filtering: maximizes pre-whitening to ensure
         * stability before tonality estimation is implemented. */
        ...
        for (int e = 0; e < n_env; e++)
        {
            ...
            {
                int k0 = sbr->bandEdges[b], k1 = clamp_int(sbr->bandEdges[b+1], k0 + 1, SBR_QMF_BANDS_64);
                int eslots = (n_env == 1) ? sampled : sampled / 2;
                faac_real E = 0.0f;
                if (n_env == 1) for (int k = k0; k < k1; k++) E += bandHalfE[ch][0][k] + bandHalfE[ch][1][k];
                else for (int k = k0; k < k1; k++) E += bandHalfE[ch][e][k];

                E /= (faac_real)(eslots * (k1 - k0));
                faac_real factor = sbr->eff_amp_res ? 1.0f : 2.0f;
                int level = FAAC_LRINT(factor * (fast_log2(E + SBR_LOG_ENERGY_FLOOR) - SBR_ENV_LEVEL_LOG2_OFFSET));
                int raw = clamp_int(level, 0, sbr->eff_amp_res ? 63 : 127);
                if (prev < 0) { sbr->ch[ch].envData[e][b] = raw; prev = raw; }
                else { int d = clamp_int(raw - prev, -dlav, dlav); sbr->ch[ch].envData[e][b] = d; prev += d; }
            }
        }
        int prevN = -1;
        for (int nb = 0; nb < sbr->numNoiseBands; nb++)
        {
            int raw = 0; /* min injection noise (noise_level=0) */
            if (prevN < 0) { sbr->ch[ch].noiseData[nb] = raw; prevN = raw; }
            else { int d = clamp_int(raw - prevN, -15, 15); sbr->ch[ch].noiseData[nb] = d; prevN += d; }
        }
    }
}
...
{
    int bits = 0, n_env = sbr->numEnvelopes, n_bands = sbr->numBands;
    ...
    for (int e = 0; e < n_env; e++)
    {
        for (int b = 0; b < n_bands; b++)
        {
            ...
            if (b == 0)
            {
                int fbits = sbr->eff_amp_res ? 6 : 7;
                if (wf) PutBit(bs, clamp_int(val, 0, (1 << fbits) - 1), fbits);
                bits += fbits;
            }
...
{
    int bits = 0, n_q = sbr->numEnvelopes > 1 ? 2 : 1;
    for (int ne = 0; ne < n_q; ne++)
    {
        for (int nb = 0; nb < sbr->numNoiseBands; nb++)
        {
            ...
            if (nb == 0)
            {
                if (wf) PutBit(bs, clamp_int(val, 0, 30), 5);
                bits += 5;
            }
