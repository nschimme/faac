...
static void sbr_quantize_envelopes(SBRInfo *sbr, int nch, int kx, int k2, int sampled,
...
/* The inlined SbrQmfAnalysis fallback below only fires when sa->valid is
 * false, but SbrAnalyze (always called first) sets it unconditionally, so
 * the fallback is dead on every real call chain. */
void SbrEncode(SBRInfo *sbr, faac_real *timeDomain[MAX_CHANNELS], int numChannels, int numSamples, struct SignalAnalysis *sa)
{
    ...
    memset(bandHalfE, 0, 2 * 2 * SBR_QMF_BANDS_64 * sizeof(faac_real));
    for (int ch = 0; ch < nch; ch++) {
    ...
    sbr_adopt_envelope_grid(sbr, sa, tratio);

    int sampled = (sa && sa->valid) ? sa->sampled : (num_slots - 1) / FAAC_SBR_DECIMATION + 1;
    sbr_quantize_envelopes(sbr, nch, kx, k2, sampled, sa, bandHalfE);
}
