    ...
    for (int ch = 0; ch < numChannels; ch++) {
        ...
        for (int e = 0; e < SBR_NUM_ENVELOPES; e++) {
            ...
            for (int b = 0; b < sbr->numBands; b++) {
                ...
                float step = sbr->bs_amp_res ? 1.0f : 2.0f;
                int delta_min = sbr->bs_amp_res ? -31 : -60;
                ...
                if (prevLevel < 0) {
                    ...
                    int delta = clamp_int(raw_level - prevLevel, delta_min, delta_max);
                    sbr->envData[ch][e][b] = delta;
                    raw_level = prevLevel + delta;
                }
                prevLevel = raw_level;
            }
        }
        ...
        for (int nb = 0; nb < sbr->numNoiseBands; nb++) {
            int cur_noise = noise_level;
            if (prevNoise < 0) {
                sbr->noiseData[ch][nb] = cur_noise;
            } else {
                int delta = clamp_int(cur_noise - prevNoise, -15, 15);
                sbr->noiseData[ch][nb] = delta;
                cur_noise = prevNoise + delta;
            }
            prevNoise = cur_noise;
        }
    }
...
{
    int bits = 0;
    const SBRHuffEntry *table = sbr->bs_amp_res ? f_huff_env_3_0dB : f_huff_env_1_5dB;
    int nsyms = sbr->bs_amp_res ? F_HUFF_ENV_3_0DB_NSYMS : F_HUFF_ENV_1_5DB_NSYMS;
    int offset = sbr->bs_amp_res ? F_HUFF_ENV_3_0DB_OFFSET : F_HUFF_ENV_1_5DB_OFFSET;
    int first_bits = sbr->bs_amp_res ? 6 : 7;
