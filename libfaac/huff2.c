...
static void huff_count_all_books(CoderInfo *coder, int band, int *costs)
{
    ...
    const int * __restrict qs = (coder->qspec + coder->qoffset[band]);
    int ofs, k;

    for (k = 1; k <= 11; k++)
        costs[k] = (maxq > maxq_for_book(k)) ? -1 : 0;

    if (maxq == 0)
    {
        /* Zeros cost exactly 1 bit per tuple in spectral books */
        if (costs[1] >= 0) costs[1] = ((len + 3) >> 2);
        if (costs[2] >= 0) costs[2] = ((len + 3) >> 2);
        if (costs[3] >= 0) costs[3] = ((len + 3) >> 2);
        if (costs[4] >= 0) costs[4] = ((len + 3) >> 2);
        for (k = 5; k <= 11; k++) if (costs[k] >= 0) costs[k] = ((len + 1) >> 1);
        return;
    }

    const int b1_len = book01[40].len;
    const int b3_len = book03[0].len;
    const int b5_len = book05[40].len;
    const int b7_len = book07[0].len;
    const int b9_len = book09[0].len;
    const int b11_len = book11[0].len;
    ...
    for (ofs = 0; ofs < len; ofs += 4)
    {
        ...
        if (!(q0 | q1 | q2 | q3))
        {
            if (costs[1] >= 0) costs[1] += b1_len;
            if (costs[2] >= 0) costs[2] += b1_len;
            if (costs[3] >= 0) costs[3] += b3_len;
            if (costs[4] >= 0) costs[4] += b3_len;
            if (costs[5] >= 0) costs[5] += 2 * b5_len;
            if (costs[6] >= 0) costs[6] += 2 * b5_len;
            if (costs[7] >= 0) costs[7] += 2 * b7_len;
            if (costs[8] >= 0) costs[8] += 2 * b7_len;
            if (costs[9] >= 0) costs[9] += 2 * b9_len;
            if (costs[10] >= 0) costs[10] += 2 * b9_len;
            if (costs[11] >= 0) costs[11] += 2 * b11_len;
            continue;
        }
        ...
        if (costs[1] >= 0 || costs[2] >= 0 || costs[3] >= 0 || costs[4] >= 0)
        {
            ...
            if (costs[1] >= 0) costs[1] += book01[idx].len;
            if (costs[2] >= 0) costs[2] += book02[idx].len;
            if (costs[3] >= 0 || costs[4] >= 0)
            {
                int a2 = abs(q2), a3 = abs(q3);
                int aidx = 27 * a0 + 9 * a1 + 3 * a2 + a3;
                int signbits = s0 + s1 + s2 + s3;
                if (costs[3] >= 0) costs[3] += book03[aidx].len + signbits;
                if (costs[4] >= 0) costs[4] += book04[aidx].len + signbits;
            }
        }

        if (costs[5] >= 0) costs[5] += (book05[9 * q0 + q1 + 40].len + book05[9 * q2 + q3 + 40].len);
        if (costs[6] >= 0) costs[6] += (book06[9 * q0 + q1 + 40].len + book06[9 * q2 + q3 + 40].len);
        if (costs[7] >= 0 || costs[8] >= 0 || costs[9] >= 0 || costs[10] >= 0 || costs[11] >= 0)
        {
            int a2 = abs(q2), a3 = abs(q3);
            int idx0 = 8 * a0 + a1, idx1 = 8 * a2 + a3;
            int sign0 = s0 + s1, sign1 = s2 + s3;
            if (costs[7] >= 0) costs[7] += book07[idx0].len + sign0 + book07[idx1].len + sign1;
            if (costs[8] >= 0) costs[8] += book08[idx0].len + sign0 + book08[idx1].len + sign1;
            idx0 = 13 * a0 + a1; idx1 = 13 * a2 + a3;
            if (costs[9] >= 0) costs[9] += book09[idx0].len + sign0 + book09[idx1].len + sign1;
            if (costs[10] >= 0) costs[10] += book10[idx0].len + sign0 + book10[idx1].len + sign1;
            if (costs[11] >= 0)
            {
                int dummy;
                costs[11] += book11[17 * ((a0 > 16) ? 16 : a0) + ((a1 > 16) ? 16 : a1)].len + sign0;
                if (a0 >= 16) costs[11] += escape(a0, &dummy);
                if (a1 >= 16) costs[11] += escape(a1, &dummy);
                costs[11] += book11[17 * ((a2 > 16) ? 16 : a2) + ((a3 > 16) ? 16 : a3)].len + sign1;
