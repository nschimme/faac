                              ...
                              int bal, int linked, BitStream *bs, int ch, bool write)
{
    ...
    for (int e = 0; e < fd->numEnvelopes; e++) {
        const int *ref = e ? env[e - 1] : sc->lastEnv;
                              ...
                              int sendHeader, int coupled, bool write)
{
    ...
    int noiseLinked = !sendHeader && sc0->lastNb && sc0->lastCoupled == coupled;
    int linked = noiseLinked && sc0->lastNb == sbr_env_bands(sbr, fd) && sc0->lastAmpRes == fd->eff_amp_res;
    ...
    /* The reference advances only on the real write, so the sizing pass
     * makes the same choices. */
    if (write) {
        int nb = sbr_env_bands(sbr, fd);
        for (int ch = 0; ch < nch; ch++) {
            SBRChannel *sc = &sbr->ch[ch0 + ch];
            memcpy(sc->lastEnv, env[ch][fd->numEnvelopes - 1], nb * sizeof(int));
            sc->lastNb = nb;
            sc->lastAmpRes = fd->eff_amp_res;
            sc->lastCoupled = coupled;
        }
    }
    return bits;
}
