...
static void write_sbr_header(const SBRInfo *sbr, BitAccumulator *acc)
{
    AccumPutBits(acc, sbr->bs_amp_res,     1);
    AccumPutBits(acc, sbr->bs_start_freq,  4);
    AccumPutBits(acc, sbr->bs_stop_freq,   4);
    AccumPutBits(acc, sbr->bs_xover_band,  3);
    AccumPutBits(acc, 0,                   2);
    AccumPutBits(acc, 1,                   1);
    AccumPutBits(acc, 0,                   1);
    AccumPutBits(acc, 0,                   2);
    AccumPutBits(acc, sbr->bs_alter_scale, 1);
    AccumPutBits(acc, 0,                   2);
}
...
static int write_sbr_grid(const SBRInfo *sbr, const SbrFrameData *fd, BitAccumulator *acc, bool write)
{
    int num_env = fd->numEnvelopes;
    int bits = 2;

    if (write) AccumPutBits(acc, fd->frameClass, 2);
    if (fd->frameClass == SBR_FRAME_CLASS_VARFIX) {
        if (write) {
            AccumPutBits(acc, fd->tEnv[0], 2);
            AccumPutBits(acc, num_env - 1, 2);
            for (int i = 0; i < num_env - 1; i++)
                AccumPutBits(acc, (fd->tEnv[i + 1] - fd->tEnv[i] - 2) / 2, 2);
        }
        int ptr_len = sbr_ceil_log2[num_env];
        if (write) {
            AccumPutBits(acc, fd->bsPointer, ptr_len);
            for (int i = 0; i < num_env; i++)
                AccumPutBits(acc, sbr->bs_freq_res, 1);
        }
        bits += 4 + 2 * (num_env - 1) + ptr_len + num_env;
    } else {
        if (write) {
            AccumPutBits(acc, num_env > 1 ? 1 : 0, 2);
            AccumPutBits(acc, sbr->bs_freq_res, 1);
        }
        bits += 3;
    }
...
static int write_sbr_envelope(const SBRInfo *sbr, const SbrFrameData *fd, BitAccumulator *acc, int ch, bool write)
{
...
static int write_sbr_data(const SBRInfo *sbr, const SbrFrameData *fd, BitAccumulator *acc, int id_aac, const int *ch_indices, bool write)
{
    ...
    if (write) AccumPutBits(acc, 0, lead_len);

    for (int ch = 0; ch < nch; ch++)
        bits += write_sbr_grid(sbr, fd, acc, write);

    if (write) {
        for (int ch = 0; ch < nch; ch++)
            AccumPutBits(acc, 0, fd->numEnvelopes + n_q);
        for (int ch = 0; ch < nch; ch++)
            AccumPutBits(acc, fd->ch[ch_indices[ch]].invfMode, 2);
    }

    for (int ch = 0; ch < nch; ch++)
        bits += write_sbr_envelope(sbr, fd, acc, ch_indices[ch], write);

    if (write) {
        for (int ch = 0; ch < nch; ch++)
            for (int ne = 0; ne < n_q; ne++)
                AccumPutBits(acc, clamp_int(fd->ch[ch_indices[ch]].noiseData[ne][0], 0, 30), 5);
        AccumPutBits(acc, 0, flags_len);
    }
...
{
    int bits = 5 + (sendHeader ? 21 : 0);
    BitAccumulator acc = {0};

    if (write) {
        AccumBegin(&acc, bs);
        AccumPutBits(&acc, (SBR_EXT_TYPE_SBR << 1) | (sendHeader & 1), 5);
        if (sendHeader) write_sbr_header(sbr, &acc);
    }

    bits += write_sbr_data(sbr, fd, &acc, id_aac, ch_indices, write);
