...
void FaacPsyInitContext(FaacPsyContext *ctx, int sample_rate,
    ...
    for (sfb = 0; sfb < num_cb_long; sfb++) {
        ...
        /* ATH scaling:
           Offset by -40.0dB. This is a balanced middle ground to avoid over-masking
           while maintaining bitrate efficiency.
        */
        ctx->ath_long[sfb] = pow(10.0, (interpolate_ath(center_freq) - 40.0) / 10.0) * (cb_width_long[sfb]);
    ...
    for (sfb = 0; sfb < num_cb_short; sfb++) {
        ...
        ctx->ath_short[sfb] = pow(10.0, (interpolate_ath(center_freq) - 40.0) / 10.0) * (cb_width_short[sfb]);
