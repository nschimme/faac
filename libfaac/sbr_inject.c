...
struct SbrInject *loadit(void) {
 ...
 if(!path||!fs||!(fp=fopen(path,"r"))) return NULL; in=calloc(1,sizeof(*in)); if(!in){fclose(fp);return NULL;} in->fields=fields(fs); in->fields|=in->fields?IF_HDR:0; in->offset=(unsigned)strtol(getenv("FAAC_SBR_INJECT_OFFSET")?getenv("FAAC_SBR_INJECT_OFFSET"):"0",NULL,0); in->f=calloc(INJECT_FRAMES,sizeof(*in->f)); if(!in->f){fclose(fp);free(in);return NULL;}
