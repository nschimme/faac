...
struct SbrInject { unsigned fields, offset; InjectFrame *f; int warned; };
...
struct SbrInject *loadit(void) {
 ...
 if(!path||!(fp=fopen(path,"r"))) return NULL; in=calloc(1,sizeof(*in)); if(!in){fclose(fp);return NULL;} in->fields=fs?fields(fs):IF_GRID; in->offset=(unsigned)strtol(getenv("FAAC_SBR_INJECT_OFFSET")?getenv("FAAC_SBR_INJECT_OFFSET"):"0",NULL,0); in->f=calloc(INJECT_FRAMES,sizeof(*in->f)); if(!in->f){fclose(fp);free(in);return NULL;}
 ...
 else if(line[0]=='G' && sscanf(line,"G %u %u",&fr,&ch)==2 && fr<INJECT_FRAMES && ch<2) { InjectChannel*c=&in->f[fr].ch[ch]; char *p=line; int n=0; while((p=strchr(p,' '))&&n<5){p++;n++;} if(p){c->ptr=atoi(p);p=strchr(p,' ');for(int i=0;p&&i<=SBR_MAX_ENVELOPES;i++,p=strchr(p,' '))c->t[i]=atoi(++p);} }
 } fclose(fp); return in;
}
int SbrInjectGetGrid(SBRInfo *s, SignalAnalysis *sa, int stream_frame) {
    ...
    int n = stream_frame + (int)in->offset;
    if (n < 0 || n >= INJECT_FRAMES || !in->f[n].ch[0].seen) return 0;
    InjectChannel *c0 = &in->f[n].ch[0];
    if (!c0->seen) return 0;
    sa->frameClass = c0->cls;
    sa->numEnvelopes = c0->le;
    sa->bsPointer = c0->ptr;
    for (int i = 0; i <= c0->le; i++) sa->tEnv[i] = c0->t[i];
    return 1;
}

int SbrInjectApply(SBRInfo *s, SbrFrameData *fd, int stream_frame, int channels) { struct SbrInject *in=s->inject; if (!in) return 0; int n=stream_frame+(int)in->offset; if(n<0||n>=INJECT_FRAMES||!in->f[n].ch[0].seen)return 0; InjectFrame*z=&in->f[n]; for(int ch=0;ch<channels;ch++){InjectChannel*c=&z->ch[ch];if(!c->seen)continue;if(in->fields&IF_GRID){fd->frameClass=c->cls;fd->numEnvelopes=c->le;fd->bsPointer=c->ptr;memcpy(fd->tEnv,c->t,sizeof(fd->tEnv));memcpy(fd->freqResEnv,c->freq,sizeof(fd->freqResEnv));}if(in->fields&IF_ENV)memcpy(fd->ch[ch].envData,c->e,sizeof(c->e));if(in->fields&IF_NOISE)memcpy(fd->ch[ch].noiseData,c->q,sizeof(c->q));if(in->fields&IF_INVF)memcpy(fd->ch[ch].invfMode,c->invf,sizeof(c->invf));if(in->fields&IF_HARM){fd->ch[ch].addHarmonicFlag=c->harmflag;memcpy(fd->ch[ch].addHarmonic,c->harm,sizeof(c->harm));}} return 1; }
