...
1. **HE-AAC v1 & HE-AAC v2 Speedup**: `faad` is **4.05x - 4.70x faster** than `faad2` and **3.56x - 4.12x faster** than `ffmpeg` on HE-AAC v1 audio decoding, and **1.21x - 2.93x faster** on HE-AAC v2 (Parametric Stereo) decoding.
2. **Binary Footprint Efficiency**: `libfaad.so` requires only **47.7 KB** of `.text` code space—an **83.4% reduction** compared to `libfaad2.so` (287.1 KB) and **65.2% smaller** than RealNetworks `libhelix-aac` (137.0 KB).
