...
- **Speed & Quality Verification:**
  - **HE-AAC QMF Speedup:** **+32.1% to +37.2%** throughput speedup preserved on low-bitrate HE-AAC scenarios (`16k_mono_16k`, `48k_stereo_24k`).
  - **Perceptual MOS Quality:** **100.0% MD5 bit-exact match** across all 52 test cases (0 regressions).

---

## Scenario Performance Summary (`/opt/faac-benchmark`)

| Scenario | Mode | Zimtohrli MOS Δ | Throughput Δ vs Master | Bitstream MD5 Match |
| :--- | :---: | :---: | :---: | :---: |
| `16k_mono_16k` | HE-AAC v1 | +0.000 | **+34.1% 🚀** | 100% Match |
| `16k_mono_40k` | HE-AAC v1 | +0.000 | **+7.9% 🚀** | 100% Match |
| `48k_stereo_24k` | HE-AAC v1 | +0.000 | **+32.1% 🚀** | 100% Match |
| `48k_stereo_32k` | HE-AAC v1 | +0.000 | +1.8% | 100% Match |
| `48k_stereo_40k` | HE-AAC v1 | +0.000 | +2.0% | 100% Match |
| `48k_stereo_48k` | HE-AAC v1 | +0.000 | +3.9% | 100% Match |
| `48k_stereo_56k` | LC-AAC | +0.000 | +1.5% | 100% Match |
| `48k_stereo_64k` | LC-AAC | +0.000 | -0.6% | 100% Match |
| `48k_stereo_96k` | LC-AAC | +0.000 | +1.7% | 100% Match |
| `48k_stereo_128k` | LC-AAC | +0.000 | -0.4% | 100% Match |
| `48k_stereo_160k` | LC-AAC | +0.000 | +1.2% | 100% Match |
| `48k_stereo_192k` | LC-AAC | +0.000 | +2.0% | 100% Match |
| `48k_stereo_256k` | LC-AAC | +0.000 | +4.5% | 100% Match |

---

## Conclusion & Recommendation

The Lever 2 cold-path optimization delivers the optimal balance: it achieves **maximum HE-AAC throughput (+32% to +37%)** while shrinking candidate size overhead down to a minimal **+200 bytes (+0.25%)**. This combination passes all size, speed, and perceptual quality criteria.
